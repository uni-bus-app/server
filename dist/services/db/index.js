...
module.exports = class UoPBackDB {
  constructor(options) {
    this.con = mysql.createConnection(options); // this.con.connect(err => console.log(err));
  }

  query(sql, params = [], callback = null) {
    if (callback === null) {
      this.con.query(sql, params, (err, results, fields) => {// console.log(sql, results, '\n\n\n');
      });
    } else {
  ...
  test_insert_statements() {
    ...
    this.test_query('stop', SQL_STATEMENTS['stop_add'], ['University of Portsmouth', '#1']);
    this.test_query('stop', SQL_STATEMENTS['stop_add'], ['Elm Grove', '#2']);
    this.test_query('arrival', SQL_STATEMENTS['arrival_add'], [1, 1, '1234', true, 'exception']);
    this.test_query('arrival', SQL_STATEMENTS['arrival_add'], [2, 1, '2345', false, 'collection only']);
    this.test_query('daterange', SQL_STATEMENTS['daterange_add'], ['2020/02/01', '2020/01/01', true]);
    this.test_query('daterange', SQL_STATEMENTS['daterange_add'], ['2020/03/01', '2020/02/01', false]);
  ...
  test_update_statements() {
    ...
    this.test_query('stop', SQL_STATEMENTS['stop_edit'], ['Kings Theatre', '#2', 1]);
    this.test_query('arrival', SQL_STATEMENTS['arrival_edit'], [2, 1, '4321', false, 'geezer', 1]);
    this.test_query('daterange', SQL_STATEMENTS['daterange_edit'], ['2020/12/01', '2020/11/01', false, 1]);
      ...
      uopdf.getStopsAndTimes('u1.pdf', null).subscribe(data => {
        ...
        times.map((time_row, i) => {
          time_row.forEach(time => {
            let rollover = false;
            if (time.slice(0, 2) == "00") rollover = true;
            this.query(SQL_STATEMENTS['arrival_add'], [1, i % stops.length + 1, time, rollover, '']);
          });
        });
  ...
  test_insert_real_dates() {
    ...
    uopdates.getPublicHolidayDates(2019).subscribe(data => {
      data.dateRanges.forEach(dateRange => {
        ...
        this.query(SQL_STATEMENTS['daterange_add'], [start_date, end_date, whitelist]);
      });
    ...
    for (let i = 0; i < 54; i++) {
      ...
      this.query(SQL_STATEMENTS['daterange_add'], [saturday, sunday, false]);
